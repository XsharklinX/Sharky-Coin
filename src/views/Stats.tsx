...
export function Stats({ txns, mkey }: ViewProps) {
  ...
  const comparison = months.map((month, index) => ({ label: month.label, actual: month.expense, anterior: previous[index]?.expense ?? 0 }))
  const categoryData = byCategory(yearTx, 'expense', categories).map(item => ({ name: item.category.name, size: item.amount, fill: item.category.color || '#8a96ac' }))
